...
class User extends Authenticatable
{
    ...
    public function transformEmployee($user_id, $start_date, $end_date)
    {
        ...
        while (strtotime($start_date) <= strtotime($end_date)) {
            ...
            $check_ins = CheckIn::where('user_id', $user_id)->where('shift_date', $start_date)->get();
 ...
 $activities  = ActivityTracker::where('user_id', $id)->whereDate('activity_time', '=',  $current_date)->get();
       ...
       $employee_attendance['screen-time']=$formattedTime;
        //screen time logic ends here                                            
                ...
                // Set attendance status (shift-aware)
                $today_shift_date = ShiftHelper::resolveShiftDate($shift_type);
            ...
            if (Holiday::where('start_date', '<=', $current_date)
                ...
                // Check-in related comments — shift-aware late cutoff
                if ($employee_attendance['check_in'] != "Yet to Check-in" && $employee_attendance['check_in'] != "Not Checked In") {
                    $late_cutoff_str  = ($shift_type === 'night') ? '07:00 PM' : '11:00 AM';
                    $late_cutoff_dt   = DateTime::createFromFormat('h:i A', $late_cutoff_str);
                    $check_in_dt      = DateTime::createFromFormat('h:i A', $employee_attendance['check_in']);

                    if ($check_in_dt && $check_in_dt > $late_cutoff_dt) {
